...
| **Portfolio Lab** | Enter the **tickers** the client owns and the portfolio you’d move them to — as **% weights or $ amounts** (toggle per portfolio; the paste importer detects which your statement uses). ETFs, mutual funds, and single stocks resolve against a built-in library of ~1,140 symbols (with autocomplete, statement-paste import, and BRK.B/BRK-B style symbol matching) mapped to asset-class capital-market assumptions (expected return, volatility, correlations — all editable; unknown tickers just pick a class). **Two return bases, your choice:** *Planning (forward)* maps each ticker to its asset class and uses conservative long-run capital-market assumptions; *Historical (trailing 20-yr or since-inception)* uses each fund’s **real, dividend-adjusted total return** from its actual price history — a benchmark that shows what the holdings actually did (past performance, clearly labeled). Roughly **1,100 symbols** carry real history baked in (so it still works offline); a fund too young for the window uses the history it has, and one with no data falls back to its asset-class assumption. In *Planning* mode a brand-new ETF is modeled just like an established one — no track-record minimum — and any row’s return/volatility can be overridden by hand. Then Monte Carlo both portfolios **through the client’s entire financial plan** (spending, Social Security, pension, taxes, RMDs) and compare success rates, median ending estate, and worst-decile longevity side by side — run a **retirement-withdrawal stress test** (inflation-adjusted income with the sustainable draw found at 90% confidence), or a standalone “grow a lump sum” test. In “through the plan,” the portfolio’s expected return and volatility **replace** the plan’s pre-/in-retirement return assumptions; one click adopts them as the plan’s assumptions everywhere (and clears any conflicting per-account growth rates so nothing silently overrides them). |
...
The Portfolio Lab’s **Historical** return basis shows **past performance** (real,
dividend-adjusted total returns from each fund’s price history) as a benchmark —
it is explicitly *not* a projection or a guarantee, and trailing returns tend to
run well above forward-looking estimates because the last two decades were an
unusually strong market. Use it to frame the conversation, and keep **Planning
(forward)** for the numbers you set client expectations on.
